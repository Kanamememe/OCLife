...
const version=JSON.parse(read('version.json')).version,index=read('index.html'),photo=read('js/photo-moments-v2.js'),story=read('js/story-reactions.js'),identity=read('js/moment-card-identity-v2.js'),health=read('js/module-health.js');
...
assert(index.includes(`js/moment-card-identity-v2.js?v=${version}`),'moment card identity module is not loaded');
...
for(const marker of ['sort((a,b)=>(b.at||0)-(a.at||0))','card.dataset.momentId=m.id','OCLifeMomentCardIdentity'])assert(identity.includes(marker),`moment identity missing ${marker}`);
for(const marker of ['OCLifeStoryReactions','OCLifeMomentCardIdentity','publishMoment','characterAllowed'])assert(health.includes(marker),`module health missing ${marker}`);
