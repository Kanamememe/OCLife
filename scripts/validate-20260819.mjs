...
assert(index.includes(`js/story-reactions-context-fix.js?v=${version}`),'story reactions context fix is not loaded');
...
for(const marker of ['#storyReactionOpen','stopImmediatePropagation','OCLifeStoryReactionContext'])assert(storyContext.includes(marker),`story context fix missing ${marker}`);
...
for(const marker of ['Story reactions WebKit smoke test passed.','story-reaction','主線評價測試','story reaction toolbar reused the previously opened world'])assert(storySmoke.includes(marker),`story reactions smoke missing ${marker}`);
