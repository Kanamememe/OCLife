(function(){
'use strict';
const VERSION='1.4.0',S=window.OCLifeStore;
const KEY='oclife_auto_life_settings';
const defaults={enabled:true,minMinutes:8,maxMinutes:22,chat:true,moment:true,event:true,status:true,chatWeight:55,momentWeight:25,eventWeight:20,quietStart:1,quietEnd:7,catchUp:true,toast:true};
let timer=null,busy=false;
function settings(){try{return{...defaults,...JSON.parse(localStorage.getItem(KEY)||'{}')}}catch(_){return{...defaults}}}
function save(x){try{localStorage.setItem(KEY,JSON.stringify({...settings(),...x}))}catch(_){}schedule()}
...
function scheduledAutomationAllowed(wid){const w=S.get('worlds',wid);return !w?.sharedWorldId||window.OCLifeSharedWorlds?.canAutomate?.(wid)!==false}
async function run(reason='timer',forcedType=''){
 const s=settings(),wid=worldId();if(!wid){toast('目前沒有進入任何世界',{error:true});return false}if(busy){toast('上一個生成還在進行中');return false}if(reason!=='manual'&&(!s.enabled||quiet(s)))return false;if(reason!=='manual'&&!scheduledAutomationAllowed(wid))return false;
 const chars=S.all('characters',{worldId:wid});if(!chars.length){toast('這個世界還沒有角色',{error:true});return false}busy=true;toast(forcedType==='chat'?'正在生成角色聊天…':forcedType==='moment'?'正在生成角色動態…':forcedType==='event'?'正在生成生活事件…':'正在生成新的生活紀錄…',{sticky:true});
 try{
  ...
  else if(type==='event')payload=await window.OCLifeAutoEvents?.generate?.(wid);
  else{const cw=s.chat?Math.max(0,Number(s.chatWeight)||0):0,mw=s.moment?Math.max(0,Number(s.momentWeight)||0):0,ew=s.event?Math.max(0,Number(s.eventWeight)||0):0,total=cw+mw+ew;if(total>0){const r=Math.random()*total;if(r<cw&&chars.length>1){payload=await window.OCLifeSimulator.autonomousChat(wid);type=payload?.length?'chat':'status'}else if(r<cw+mw&&s.moment){payload=await window.OCLifeSimulator.autonomousMoment(wid);type=payload?'moment':'status'}else if(s.event){payload=await window.OCLifeAutoEvents?.generate?.(wid);type=payload?'event':'status'}}else type='status'}
  if(forcedType&&!payload)throw new Error('AI 沒有產生有效內容');
  let text='';if(type==='chat'){const ns=namesFromMessages(payload);text=ns.length>=2?`✓ ${ns.slice(0,2).join(' 和 ')}的聊天已生成`:'✓ 角色聊天已生成'}else if(type==='moment'){const c=S.get('characters',payload.characterId);text=`✓ ${c?.name||'角色'}的新動態已生成`}else if(type==='event'){text=`✓ 新事件：${payload?.title||'生活事件'}`}else if(statusTouched)text='✓ 角色的「現在」狀態已更新';else text='✓ 生成完成';
...
function catchUp(){const s=settings(),wid=worldId();if(!s.enabled||!s.catchUp||!wid||!scheduledAutomationAllowed(wid))return;const k='oclife_auto_last_'+wid,last=Number(localStorage.getItem(k)||0),now=Date.now();try{localStorage.setItem(k,String(now))}catch(_){}if(last&&now-last>Math.max(15,s.minMinutes)*60000&&!quiet(s))setTimeout(()=>run('return'),1200)}
...
window.OCLifeAutoLife={version:VERSION,settings,save,runNow:()=>run('manual'),runChat:()=>run('manual','chat'),runMoment:()=>run('manual','moment'),runEvent:()=>run('manual','event'),schedule,toast,hideToast};
