...
function install(){setTimeout(()=>check(true),2300);window.addEventListener('error',e=>{if(e?.filename&&/\/js\//.test(e.filename))console.error('[OC Life] script error',e.filename,e.message)},true);window.addEventListener('unhandledrejection',e=>console.error('[OC Life] unhandled promise rejection',e.reason))}
