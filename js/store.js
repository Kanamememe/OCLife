...
// Names are identity labels, so simplified/traditional Chinese variants must resolve to one OC.
const VARIANT_PAIRS=['蕭萧','逸逸','沈沈','祁祁','霜霜','黎黎','澈澈','若若','羽羽','鴦鸯','鴛鸳','幽幽','璟璟','宵宵','麗丽','龍龙','風风','雲云','葉叶','劉刘','陳陈','張张','趙赵','錢钱','孫孙','吳吴','鄭郑','馮冯','衛卫','蔣蒋','韓韩','楊杨','謝谢','蘇苏','羅罗','鐘钟','鍾钟','顧顾','許许','鄧邓','鄒邹','盧卢','賈贾','賴赖','龔龚','嚴严','喬乔','闕阙','閻阎','簡简','萬万','樂乐','鄺邝','譚谭','黃黄','藍蓝','綠绿','紅红','夢梦','靈灵','星星','曉晓','陽阳','陰阴','愛爱','戀恋','歡欢','憂忧','寧宁','靜静','華华','國国','東东','門门','馬马','鳥鸟','魚鱼','貓猫','狼狼','狐狐','劍剑','獵猎','騎骑','車车','賽赛','場场','隊队','醫医','師师','學学','會会','長长','頭头','體体','關关','係系','親亲','敵敌','對对','與与','為为','從从','這这','個个','們们','裡里','時时','間间','開开','發发','現现','實实','進进','過过','後后','還还','說说','話话','點点','應应','該该'];
...
function mergeEmpty(existing,input){const patch={};for(const [k,v] of Object.entries(input||{})){if(['id','createdAt','updatedAt','worldId','name'].includes(k))continue;if((existing[k]==null||String(existing[k]).trim()==='')&&v!=null&&String(v).trim()!=='')patch[k]=v}return patch}
...
function createCharacter(input){
  const worldId=input.worldId,name=input.name||'未命名角色';
  const existing=worldId?findCharacterByName(worldId,name):null;
  if(existing){const patch=mergeEmpty(existing,input);if(Object.keys(patch).length)update('characters',existing.id,patch);window.dispatchEvent(new CustomEvent('oclife:duplicate-character-blocked',{detail:{existingId:existing.id,existingName:existing.name,inputName:name}}));return get('characters',existing.id)}
  const c=add('characters',{worldId,name,emoji:input.emoji||'◉',gender:input.gender||'',age:input.age||'',appearance:input.appearance||'',personality:input.personality||'',identity:input.identity||'',speech:input.speech||'',relationships:input.relationships||'',habits:input.habits||'',background:input.background||'',abilities:input.abilities||'',boundaries:input.boundaries||'',notes:input.notes||'',avatarData:input.avatarData||'',relationshipLinks:Array.isArray(input.relationshipLinks)?input.relationshipLinks:[]});
  add('statuses',{worldId:c.worldId,characterId:c.id,place:'未設定',activity:'正在休息',mood:'平靜',withIds:[],updatedAt:Date.now()});return c
}
