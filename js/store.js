...
const seed={version:1,settings:{provider:'none',baseUrl:'',apiKey:'',model:'',autoSimulate:true},worlds:[],characters:[],statuses:[],chats:[],messages:[],moments:[],events:[]};
const uid=(p='id')=>`${p}_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;
const clone=v=>JSON.parse(JSON.stringify(v));
function load(){try{const raw=JSON.parse(localStorage.getItem(KEY)||'null');const data=raw&&raw.version?Object.assign(clone(seed),raw):clone(seed);for(const k of ['worlds','characters','statuses','chats','messages','moments','events'])if(!Array.isArray(data[k]))data[k]=[];return data}catch(_){return clone(seed)}}
let db=load();
...
function deleteWorld(worldId){const chatIds=all('chats',{worldId}).map(x=>x.id);db.worlds=db.worlds.filter(x=>x.id!==worldId);db.characters=db.characters.filter(x=>x.worldId!==worldId);db.statuses=db.statuses.filter(x=>x.worldId!==worldId);db.chats=db.chats.filter(x=>x.worldId!==worldId);db.messages=db.messages.filter(x=>!chatIds.includes(x.chatId));db.moments=db.moments.filter(x=>x.worldId!==worldId);db.events=db.events.filter(x=>x.worldId!==worldId);save()}
function exportData(){return new Blob([JSON.stringify(db,null,2)],{type:'application/json'})}
function importData(obj){if(!obj||!Array.isArray(obj.worlds)||!Array.isArray(obj.characters))throw new Error('不是有效的 OC Life 備份');db=Object.assign(clone(seed),obj,{version:1});for(const k of ['worlds','characters','statuses','chats','messages','moments','events'])if(!Array.isArray(db[k]))db[k]=[];save()}
